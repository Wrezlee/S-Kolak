...
class LaporanController extends Controller
{
    ...
    /**
     * Tampilan Laporan Operator — persis sama dengan Laporan Admin (Ringkasan
     * Eksekutif, Laporan Detail, Grafik & Visualisasi + filter tahun/bulan),
     * bedanya seluruh data di sini dibatasi hanya milik operator yang login.
     */
    public function index(Request $request)
    {
        ...
        $allItems = NeracaPangan::with('komoditas')
            ->where('diinput_oleh', $operatorId)
            ->orderByDesc('periode')
            ->get();

        // ── Ringkasan Eksekutif (selalu dari seluruh data operator, tidak tergantung filter) ──
        ...
        $laporanBulanan = $this->buildLaporanPeriode($operatorId, $filters['tahun'], $filters['bulan']);
        $laporanTahunan = $this->buildLaporanRataRata($operatorId, $filters['tahun']);

        // ── Grafik & Visualisasi (dari seluruh data operator) ──
        ...
        return view('operator.laporan', [
            ...
            'notifCount'        => Notifikasi::where('user_id', $operatorId)
                ->where('dibaca', false)
                ->count(),
            'totalEntriSaya'    => $allItems->count(),
        ]);
    }

    /**
     * Versi cetak (print-friendly) dari Laporan Operator, memakai filter query string
     * yang sama (tahun & bulan). Sama seperti Cetak PDF di Laporan Admin: dicetak
     * lewat dialog print browser (window.print()), tanpa dependency PDF tambahan.
     */
    ...
    /**
     * Ambil & normalisasi filter dari query string. Sama seperti Laporan Admin:
     * hanya tahun & bulan. Default ke bulan & tahun berjalan.
     */
    ...
    /**
     * Susun baris Laporan untuk satu periode (bulan+tahun) milik operator — satu
     * baris per komoditas (semua komoditas master ditampilkan, walau operator
     * belum punya data untuk periode tsb).
     */
    private function buildLaporanPeriode(int $operatorId, string $tahun, string $bulan): array
    {
        ...
        if ($tahun && $bulanNum) {
            ...
            $items = NeracaPangan::where('diinput_oleh', $operatorId)
                ->where('status', 'valid')
    ...
    /**
     * Susun baris Laporan rata-rata tahunan milik operator — satu baris per
     * komoditas, tiap kolom dirata-rata dari entri valid operator sepanjang
     * tahun terpilih (Januari s.d. Desember).
     */
    private function buildLaporanRataRata(int $operatorId, string $tahun): array
    {
        $items = collect();
        if ($tahun) {
            $items = NeracaPangan::where('diinput_oleh', $operatorId)
