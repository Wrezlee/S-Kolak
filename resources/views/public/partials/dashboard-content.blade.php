        ...
        @if ($detailData)
            ...
            <div class="mt-4 rounded-xl {{ $infoBoxStyle['box'] }} border p-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="font-bold {{ $infoBoxStyle['text'] }}">{{ number_format($detailData['nilai_neraca'], 0, ',', '.') }} Ton</p>
                </div>
            ...
            <tbody>
                @forelse ($tableRows as $row)
                    ...
                    <tr class="border-b border-slate-100 hover:bg-blue-50/30 transition-colors">
                        ...
                        <td class="px-4 py-3 text-right tabular-nums font-bold text-slate-800">{{ number_format($row['nilai_neraca'], 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
