...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="h-14 border-b border-blue-100 bg-white flex items-center px-4 gap-3 flex-shrink-0 shadow-sm">
            <div class="flex-1">
                <h2 id="pageHeaderTitle" class="text-sm font-bold" style="color:#1E3A5F;">Riwayat Verifikasi</h2>
        ...
        <main id="pageContent" class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
            ...
            <div class="bg-white rounded-xl border border-blue-100 shadow-sm overflow-hidden">
                ...
                <form method="GET" class="p-4 border-b border-blue-50 flex items-center gap-3 flex-wrap">
                    ...
                    @if (request('search'))
                        <a href="{{ route('verifikator.riwayat') }}" class="text-xs text-slate-400 hover:text-slate-600">Reset</a>
                    @endif
                </form>
    ...
    <template id="riwayat-detail-{{ $n->id }}">
        ...
        <div class="rounded-xl p-4 border-2" style="border-color:{{ $nilaiTpl > 0 ? '#86EFAC' : '#FCA5A5' }}; background-color:{{ $nilaiTpl > 0 ? '#F0FDF4' : '#FEF2F2' }};">
            <div class="flex items-center justify-between">
                ...
                <p class="angka-neraca text-2xl font-bold text-black">{{ fmt_neraca($nilaiTpl) }}</p>
            </div>
        </div>
