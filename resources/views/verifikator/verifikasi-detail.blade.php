            ...
            @foreach ($menuItems as $mi)
                ...
                <a href="{{ Route::has($mi['route']) ? route($mi['route']) : '#' }}"
                   ...
                   style="{{ $isActive ? 'background-color:#2563EB; color:white; font-weight:600;' : '' }}">
                    ...
                    <span class="truncate flex-1 text-left sidebar-label">{{ $mi['label'] }}</span>
                    @if ($mi['badge'])
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main id="pageContent" class="flex-1 overflow-y-auto p-4 sm:p-6">
            <div class="max-w-2xl mx-auto space-y-5">
                ...
                <div class="text-left">
                    <a href="{{ route('verifikator.menunggu') }}" class="inline-flex items-center gap-1 text-sm text-blue-500 hover:bg-blue-100 rounded-lg px-2 py-1 -ml-2 transition-colors">
                        ← Kembali
                    </a>
                ...
                <div class="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 space-y-5">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            @foreach ($fields as $f)
                                <div class="rounded-xl p-3 border border-blue-50">
                                    ...
                                    <p class="angka-neraca text-base font-bold mt-0.5 text-black">{{ fmt_neraca($f['val']) }}</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl p-4 border-2" style="border-color:{{ $nilai > 0 ? '#86EFAC' : '#FCA5A5' }}; background-color:{{ $nilai > 0 ? '#F0FDF4' : '#FEF2F2' }};">
                        <div class="flex items-center justify-between">
                            ...
                            <p class="angka-neraca text-2xl font-bold text-black">{{ fmt_neraca($nilai) }}</p>
                        </div>
                    </div>
